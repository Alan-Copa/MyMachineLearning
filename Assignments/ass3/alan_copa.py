# -*- coding: utf-8 -*-
"""Alan_Copa_ML_2024_assignment_3.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1YZncb_mXonRki-lUrgafIJWoV00mqnmt
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        log_probs = -np.log(softmax[np.arange(n), t])
        mean_crossentropy = np.mean(log_probs)
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray, weight_decay=0.0005) -> float:
    ...
    variable_gradients = model.backward(loss_grad).variable_grads

    # variable_gradients = model.backward(loss.backward()) # no .variable_grads
...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ...
    # Other solutions
    predictions = model.forward(images.reshape(images.shape[0], -1))
...
for epoch in range(200):
    ...
    for i in range(0, len(train_indices), batch_size):
        ...
        images = train_validation_set.images[batch_indices]
        labels = train_validation_set.labels[batch_indices]
        ...
        current_loss_value += loss_value

    current_loss_value /= len(train_indices) // batch_size
    ...
    if validation_accuracy > best_validation_accuracy:
        print("New best validation accuracy: %.2f%% > %.2f%%" % (validation_accuracy, best_validation_accuracy))
